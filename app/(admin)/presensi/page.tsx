...
export default function PresensiPage() {
  ...
  const unitsSelected = useMemo(() => Object.keys(unitChecked).filter((u) => unitChecked[u]), [unitChecked])

  const pegawaiByUnit = useMemo(() => {
    if (unitsSelected.length === 0) return []
    return ALL_PEGAWAI.filter((p) => unitsSelected.includes(p.unit))
  }, [unitsSelected])

  const totalSelected = useMemo(() => Object.values(selectedEmp).filter(Boolean).length, [selectedEmp])
  ...
  function toggleUnit(u: string, checked: boolean) {
    ...
    // reset employee selection when units change
    setSelectedEmp({})
    // reset status global when no unit selected
  ...
  const [saved, setSaved] = useState<{ sesiId: string; count: number; status: Status | null } | null>(null)
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              <Button className="bg-[#003A70] hover:opacity-95" onClick={() => { // reset form untuk sesi baru
                setTanggal(''); setJamMulai('08:00'); setJamAkhir('09:00'); setDeskripsi('')
                setUnitChecked({}); setSelectedEmp({}); setStatusGlobal(null); setSaved(null); setStep(1)
