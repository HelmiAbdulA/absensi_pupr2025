...
export default function DashboardPage() {
    ...
    async function load() {
      ...
      try {
        ...
        (trendEntriesData || []).forEach(r => {
          const key = `${r.employee_id}|${r.session.tanggal}`;
        ...
        const trend = labels.map((tgl) => {
          const d = new Date(`${tgl}T00:00:00`); // Pastikan tidak ada isu timezone
  ...
  const todaySummary = useMemo(() => {
    ...
    const total = dailyFinalStatus.size;
    const count: Record<Status, number> = { HADIR: 0, IZIN: 0, SAKIT: 0, DL: 0, TK: 0 };
    dailyFinalStatus.forEach(status => { count[status]++; });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-500 mt-1">Dari {todaySummary.total} orang tercatat</p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-500 mt-1">Dari total pegawai aktif</p>
          </CardContent>
        </Card>
      </div>
        ...
        <Card className="xl:col-span-1">
          ...
          <CardContent className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <RePieChart>
                <Tooltip formatter={(v: any, n: any) => [`${v} orang`, n]} />
          ...
          <CardContent className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <ReBarChart data={trendData}>
                ...
                <YAxis />
                <Tooltip />
                <Bar dataKey="hadir" fill="#0E5AAE" radius={[6, 6, 0, 0]} name="Pegawai Hadir" />
              </ReBarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
          ...
          <div className="rounded-md border overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {loading && (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center text-sm text-slate-500 py-10">Memuat data…</TableCell>
                  </TableRow>
                )}
                {!loading && paginated.map((r) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell>
                      ...
                      {r.status === 'DL' && <Badge className="bg-green-100 text-green-800">DL</Badge>}
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                <Select value={String(pageSize)} onValueChange={(v) => setPageSize(Number(v))}>
                  <SelectTrigger className="h-8 w-20"><SelectValue placeholder={String(pageSize)} /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="flex items-center gap-1">
                <Button variant="outline" size="sm" onClick={() => setCurrentPage(1)} disabled={currentPage === 1 || loading || filtered.length === 0}><ChevronsLeft className="h-4 w-4" /></Button>
                <Button variant="outline" size="sm" onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1 || loading || filtered.length === 0}><ChevronLeft className="h-4 w-4" /></Button>
                <span className="mx-2 text-xs text-slate-600">Page {currentPage} / {totalPages}</span>
                <Button variant="outline" size="sm" onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages || loading || filtered.length === 0}><ChevronRight className="h-4 w-4" /></Button>
                <Button variant="outline" size="sm" onClick={() => setCurrentPage(totalPages)} disabled={currentPage === totalPages || loading || filtered.length === 0}><ChevronsRight className="h-4 w-4" /></Button>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-slate-500">Lihat detail rekam jejak admin di halaman <b>Log Aktivitas</b>.</p>
